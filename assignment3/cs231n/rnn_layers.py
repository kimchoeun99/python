...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for time_step in range(T):
      if time_step == 0 :
        h[:, 0, :], tcache = rnn_step_forward(x[:, 0, :], h0, Wx, Wh, b)
        cache.append(tcache)
      else :
        h[:, time_step, :], tcache = rnn_step_forward(x[:, time_step, :], 
                                                      h[:, time_step-1, :],
                                                      Wx, Wh, b)
        cache.append(tcache)
...
def rnn_backward(dh, cache):
    ...
    for time_step in range(T-1, -1, -1) :
      dx[:, time_step, :], tdprev_h, tdWx, tdWh, tdb = rnn_step_backward(tdprev_h + dh[:, time_step, :], 
                                                                        cache[time_step])
      """td_prevh is updated step by step by adding dh[:, timestep, :] and 
        then running through the function 'rnn_step_backward'. 
        The reason we are adding dh[:, timestep, :] is because 
        the total loss is the sum of loss from each hidden state (h1 ~ hT).
      """
      dWx += tdWx
      dWh += tdWh
      db += tdb
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_temp = dnext_h * o * (1 - np.square(np.tanh(next_c))) + dnext_c
    
    di = dnext_c_temp * g
    dg = i * dnext_c_temp

    dprev_c = f * dnext_c_temp
    df = dnext_c_temp * prev_c
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for time_step in range(T):
      if time_step == 0 :
        h[:, 0, :], next_c, tcache = lstm_step_forward(x[:, 0, :], h0, c0, Wx, Wh, b)
        cache.append(tcache)
      else :
        h[:, time_step, :], next_c, tcache = lstm_step_forward(x[:, time_step, :], 
                                                              h[:, time_step-1, :],
                                                              next_c, Wx, Wh, b)
        cache.append(tcache)
...
def lstm_backward(dh, cache):
    ...
    for time_step in range(T-1, -1, -1):
      dx[:, time_step, :], tdprev_h, dc, tdWx, tdWh, tdb = lstm_step_backward(
                                                        dh[:, time_step, :] + tdprev_h, 
                                                        dc, cache[time_step])
      dWx += tdWx
      dWh += tdWh
      db += tdb
