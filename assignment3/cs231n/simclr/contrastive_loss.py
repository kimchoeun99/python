...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j.T) /denominator
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        k_denom = 0
        for i in range(2*N):
          k_denom += np.exp(sim(z_k, out[i])/tau)
        k_denom -= np.exp(1/tau)
        k_nom = np.exp(sim(z_k, z_k_N)/tau)
        l_k_kn = -np.log(k_nom / k_denom)

        kn_denom = 0
        for j in range(2*N):
          kn_denom += np.exp(sim(z_k_N, out[j])/tau)
        kn_denom -= np.exp(1/tau)
        kn_nom = np.exp(sim(z_k_N, z_k)/tau)
        l_kn_k = -np.log(kn_nom / kn_denom)
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot_product = torch.sum(out_left * out_right, 1)      #(N, 1)
    #print("dot_product shape :", dot_product.shape)
    left_norm = torch.linalg.norm(out_left, dim = 1)
    #print("left_norm shape :", left_norm.shape)
    right_norm = torch.linalg.norm(out_right, dim = 1)

    dot_product /= left_norm
    dot_product /= right_norm

    pos_pairs = dot_product.reshape((-1, 1))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    dot_matrix = out.matmul(out.T)               # (2N, 2N)
    z_norm = torch.linalg.norm(out, dim = 1)  # (2N, 1)
    dot_matrix = (dot_matrix.T / z_norm).T      # divide column
    dot_matrix /= z_norm                        # divide row

    sim_matrix = dot_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, dim = 1).reshape((-1, 1))    # (2*N, 1)
    ...
    similarity = sim_positive_pairs(out_left, out_right).to(device)
    similarity = torch.cat([similarity, similarity], dim=0)
