...
def compute_saliency_maps(X, y, model):
    ...
    score.backward(torch.FloatTensor([1.0] * N))
    saliency = X.grad
    #print('saliency : ', saliency)
    saliency = saliency.abs()
    saliency, _ = torch.max(saliency, dim=1)      # max among channels
    #print("Saliency : ", saliency)
...
def make_fooling_image(X, target_y, model):
    ...
    for epoch in range(100):
        ...
        if scores.argmax(dim=1).item() == target_y:   # if the model classifies the image as class target
            break
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    s_I = model(img)[0][target_y]
    R_I = l2_reg * torch.sum((img**2)).item()
    loss = s_I - R_I
    loss.backward()
