...
def bce_loss(input, target):
    ...
    """
    neg_abs = - input.abs()
    # .clamp(min=0) : change the value in tensor which is smaller than 0 to 0
    # z * -log(sigmoid(x)) + (1 - z) * -log(1 - sigmoid(x))
      # where z : target, x : input
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * real_loss.mean() + 0.5 * fake_loss.mean()
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
      ...
      nn.Conv2d(1, 32, 5, 1),   # (32, 24, 24) # (28-5)/1 + 1 = 24
      nn.LeakyReLU(),
      nn.MaxPool2d(2, 2),       # (32, 12, 12)
      nn.Conv2d(32, 64, 5, 1),  # (64, 8, 8)   # (12-5)/1 + 1 = 8
      nn.LeakyReLU(),
